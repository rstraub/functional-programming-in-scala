...
class RNGSpec extends AnyFlatSpec with Matchers {
  ...
  "double3" should "return three random doubles" in {
    double3(
      fortyTwo
    )._1 shouldBe (0.007524831686168909, 0.5967354853637516, 0.15846728440374136)
  }
  ...
  "map2 (ex 6.6)" should "join two randoms" in {
    RNG
      .map2(double, nonNegativeInt)((d, i) => s"$d, $i")(FakeOne)
      ._1 shouldBe "4.6566128730773926E-10, 1"
  }

  "sequence (ex 6.7)" should "combine list of transitions" in {
    RNG.sequence(List(unit(1), unit(2), unit(3)))(FakeOne)._1 shouldBe List(
      1,
      2,
      3
    )
  ...
  "intsViaSequence" should "return list of n random ints" in {
    RNG.intsViaSequence(3)(fortyTwo)._1 shouldBe List(
      16159453,
      1281479697,
      340305902
    )
  ...
  "map2ViaFlatmap" should "join two randoms" in {
    RNG
      .map2ViaFlatMap(double, nonNegativeInt)((d, i) => s"$d, $i")(FakeOne)
      ._1 shouldBe "4.6566128730773926E-10, 1"
  }
